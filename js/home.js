...
Games are loaded from Firebase Realtime Database under /gameConfigs
===================================================== */

// ── Game registry ─────────────────────────────────────────
// Games are fetched from Firebase; cache locally after first load
let GAME_CONFIGS = {};
...
onAuthReady(async user => {
  ...
  // Load game configs from Firebase
  if (Object.keys(GAME_CONFIGS).length === 0) {
    try {
      const gameConfigsSnap = await firebase.database().ref('gameConfigs').once('value');
      GAME_CONFIGS = gameConfigsSnap.val() || {};
    } catch (e) {
      console.error('Failed to load game configs:', e);
      showToast('Failed to load games. Please refresh.', 'error');
      return;
    }
  }
  ...
  if (user) {
    try {
      const snap = await firebase.database().ref(`userStats/${user.uid}`).once('value');
      stats = snap.val() || {};
...
// ── Game-specific settings collectors ─────────────────────
// These functions can't be stored in Firebase (JSON doesn't support functions)
const SETTINGS_COLLECTORS = {
  codenames: () => ({
    language: document.getElementById('cn-lang')?.value || 'en',
    timerSeconds: parseInt(document.getElementById('cn-timer')?.value || '90', 10)
  }),
  werewolves: () => ({}),
  // Add settings collectors for new games here
};
...
async function _handleCreate(slug) {
  ...
  try {
    const collector = SETTINGS_COLLECTORS[slug] || (() => ({}));
    const settings = collector();
